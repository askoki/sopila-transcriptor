...
from features.helpers.data_helpers import plot_model_statistics, \
    get_train_data, write_model_statistics
...
param_cartesian_product = product(cnn_layers, num_filters, filter_size, hidden_layers)
for i, parameters in enumerate(product(param_cartesian_product)):
    ...
    name = data_name + '_' + str(parameters[0]) + '_' + str(parameters[1]) + '_' + \
        str(parameters[2]) + '_' + str(parameters[3])
    with open(os.path.join(MODEL_DIR, 'model_' + name + '.json'), 'w') as json_file:
        json_file.write(model_json)
    
    # serialize weights to HDF5
    model.save_weights(os.path.join(MODEL_DIR, 'model_' + name + '.h5'))
